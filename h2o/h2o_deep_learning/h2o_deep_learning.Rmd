...
Since there are a lot of parameters that can impact model accuracy, hyper-parameter tuning is especially important for Deep Learning. One way to do it is with grid search. For speed, we will only train on the first 10,000 rows of the training dataset.
...
- `l1` Regularization to prevent overfitting, l1 regularization constrains the absolute value of the weights ( can add stability and improve generalization, causes many weights to become 0 ).
