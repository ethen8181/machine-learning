...
def text_to_word_sequence(text, filters=base_filter(), lower=True, split=" "):
    ...
    if lower:
        text = text.lower()
    text = text.translate( maketrans({ key: split for key in filters }) )
...
class Tokenizer(object):
    def __init__(self, nb_words=None, filters=base_filter(),
        ...
        '''
        if nb_words <= 0:
            raise ValueError('number of most common words kept should be greater than 0')
        self.word_counts = {}
        self.word_docs = {}
    ...
    def fit_on_texts(self, texts):
        ...
        # map common words to index, note that index 0 is reserved as
        # padding character, hence never assigned to an existing word
        self.frequent = Counter(self.word_counts).most_common(self.nb_words)
        self.word_counts = dict(self.frequent)
        self.word_index = { w: i for i, w in enumerate(self.word_counts) }
        self.word_docs  = { k: v for k, v in self.word_docs.items() if k in self.word_index }
