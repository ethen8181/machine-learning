...
> This documentation assumes you're famaliar with R. All the codes are in "illustrative" form, you shouldn't directly use them for actual production.
...
- Convert extra white space as a single blank.  
...
The first notion we'll introduce is **Shingling**, the most effective technique of representin documents as sets. Documents are constructed by a string of characters. Given the document, its k-shingle is said to be all the possible consecutive substring of length k found within it. That might be a bit confusing, an example with k = 3 is given below : 

```{r}

# Function for shingling the document 
...
# "shingle" to our example document, with k = 3
...
As you can see from the printed out first document. By picking our k to be 3, the k-shingle of the first document consists of substrings of lenth 3. The first 3-shingle is **`r doc1[[1]][1]`** as it's the first consecutive substring of length 3 in the document, then the second 3-shingle **`r doc1[[1]][2]`** is the next 3 word long substring after removing the first word of the document. And the list goes on. 

Another thing to add is the set of k-shingle of a document consists of unique k-shingles. For example, if the first document above contains more than one **`r doc1[[1]][1]`** then it will still only appear once as the set of k-shingle for the document. 
...
Thus the matrix will have 1 in row i and column j if and only if document j contains the shingle i and 0 otherwise. Example below. ( I will use data frame instead of matrix throughout the whole documentation ).
...
- Note that unlike our toy example, these "characteristic" matrices are almost always *sparse*, therefore, in practice we usually represent these kind of matrices of 0's and 1's only by the positions in which is 1's appear. This is more space efficient.

## Jaccard Similarity

After transforming all the documents into shingle sets, and using characteristic matrix to visualize the relatinoships between our documents. The next question is how do me measure the similarity between documents ? 
...
This is equivalent to saying for two sets, its jaccard similarity is the size of their intersection divided by their union. For our "characteristic" matrix, if we restrict ourselves to only two columns (documents), then the rows can be divided into three types :
...
The following section will calculate the pairwise jaccard similarities for all three documents, and print out the original document's content to get a feel of it. One quick way to calculate pairwise distance in R is the `dist` function, which computes and returns the distance/similarity matrix between either rows or columns of a matrix/data frame. We can also define our own measures for the distance/similarity in the proxy library's database.
...
The similarity matrix d tells us that document 1 and 3 is the most similar among the three documents, looking at its original content, it seems like it does match our intuition, as both of the documents contains the substring sun is bright and the sky is blue (despite the different ordering in the context) .

Now that we have calculated the pair-wise similarity between each document in a pretty straightforward way, we're done right ?! Well, it's a yes and a no. For small datasets, this method works out perfectly fine, but imagine if we have a large number of documents to compare instead of just three, let's say the number is N. We will then be forced to do a number of $N \choose 2$ comparisons ( N choose 2 comparisons ). This is obviously not going to scale well.
...
Using our randomly permutated rows, we'll carry out the second step of the MinHash algorithm, calculating the signatures. The signature value of any column (document) is the number of the first row, in the permutated order, in which the column has a 1. 
...
For the next section, suppose your final goal is to compute the similarity of every possible pair ( for text clustering perhaps ), then it will most likely be useless to you. However, if you simply wish to find the pairs that are most likely to be similar, then stay tuned. Because in the next section we will be discussing **Locality Sensitive Hashing**, a technique that reduces the dimensionality of a high-dimension data, allowing us to do just that.
...
What locality sensitive hashing tells us is : If the signature values of two documents agree in all the rows of at least one band, then these two documents are likely to be similar and should be compared (list it as our candidate pair) . 
...
2. k-shingle, size of k ???????????
3. To note, there's also a R library `textreuse` that implements the techniques that we've walked through in this documentation. You should be able understand its vignette with no problem.

All the code (text_similarity.R) and data used in the documentation (the data folder) can be found in [this](https://github.com/ethen8181/machine-learning/blob/master/text_similarity) folder. 

You can File a new issue or leave comments of any sort [here](https://github.com/ethen8181/machine-learning/issues). This includes Feedbacks, suggestions, bug reports or room for improvements. Thank you !


## Reference

1. Mining Massive Datasets : http://infolab.stanford.edu/~ullman/mmds/ch3.pdf
2. Blog on the same topic written from a Java coder's perspective http://matthewcasperson.blogspot.tw/2013/11/minhash-for-dummies.html
