---
title: 'A RoadTrip with Linear Regression'
author: "Ming-Yu Liu"
output: 
  html_document: 
    keep_md: yes
...
From the output above, we can see that the algorithm converges at iteration `r length(record)` before the user-specified maximum iteration, with the x and formula value that's close to the original optimal value. Let's create the visualization for the process.
...
- m : the number of training examples. Here we just use all `r nrow(housingdata)` rows.
